...
const PROFILE = {
  ...
  weight: 60,   // 120 斤
  goal: 55,     // 110 斤
  note: '当前 BMI 19.4（正常）；目标 BMI 17.8 低于健康下限 18.5 → 策略是健康减脂、保肌肉、绝不节食、不推荐断食',
  appetite: '食欲差、饭量小，经常很饿但不想吃，饿过劲后更不想吃',
  pattern: '少食多餐（3 正餐 + 1~2 加餐），愿意自己做饭（做饭算轻度活动）',
  takeout: '外卖偏好简单清淡；麦当劳/肯德基可接受（需健康化改装）',
  dessert: '偶尔会想吃小蛋糕/甜品（量少）——允许小份推荐，但必须提示频率（每周 1~2 次内）并建议配无糖饮品',
  redLines: [
    '默认禁止高油/高糖/高热量（油炸、重酱烧烤、甜点奶茶、含糖饮料）',
    '只有用户明确点名想吃时，才可推荐垃圾食品，且必须附健康化改装建议（去皮、少酱、无糖、小份）',
    '甜品/蛋糕：用户明确表示偶尔会想吃，允许在加餐时段小份推荐，但必须提示频率（每周 ≤1~2 次）和搭配（无糖茶/黑咖啡）',
